...
def q(ym):
    ...
    yellow.printSchema()
    yellow = yellow.select(yellow[" pickup_datetime"].alias(  "lpep_pickup_datetime"),
                           yellow[" dropoff_datetime"].alias( "Lpep_dropoff_datetime"),
    ...
    print "LEO: GREEN", green.count()

    df = yellow.union(green)

    print "LEO: DF", df.count()
    green.unpersist()
    yellow.unpersist()
